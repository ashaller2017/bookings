...
//grabs all new reservations (0 in processed)
...
//update processed for reservation id
func (m *postgresDBRepo) UpdateProcessedForReservation(id, processed int) error {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
	query := `
		update from reservations set processed=$1 where id=$2	
	`
