...
import (
	...
	"github.com/ashaller2017/bookings/internal/forms"
	...
	"github.com/ashaller2017/bookings/internal/repository/dbrepo"
...
func (m *Repository) PostReservation(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		m.App.Session.Put(r.Context(), "error", "cant find insert reservation into database")
		http.Redirect(w, r, "/", http.StatusSeeOther)
		return
	}
...
//shows reservation in admin tool
...
func (m *Repository) AdminReservationsCalendar(w http.ResponseWriter, r *http.Request) {
	...
	firsOfMonth := time.Date(currentYear, currentMonth, 1, 0, 0, 0, 0, currentLocation)
	lastOfMonth := firsOfMonth.AddDate(0, 1, -1)
	...
	for _, x := range rooms {
		...
		for d := firsOfMonth; d.After(lastOfMonth) == false; d = d.AddDate(0, 0, 1) {
			reservationMap[d.Format("2006-01-02")] = 0
			blockMap[d.Format("2006-01-02")] = 0
		}
		restrictions, err := m.DB.GetRestrictionsForRoomByDate(x.ID, firsOfMonth, lastOfMonth)
		if err != nil {
			helpers.ServerError(w, err)
			return
		}
...
func (m *Repository) AdminPostShowReservation(w http.ResponseWriter, r *http.Request) {
	...
	if year == "" {
		...
		http.Redirect(w, r, fmt.Sprintf("/admin/reservations-calendar?y=%s&%s", year, month), http.StatusSeeOther)
...
//handles post reservation calendar
func (m *Repository) AdminPostReservationsCalendar(w http.ResponseWriter, r *http.Request) {
	...
	form := forms.New(r.PostForm)
	for _, x := range rooms {
		//get block map and loop through to find entry that doesnt exist in posted data and
		//restriction id>0 then it is a block to remove
		curMap := m.App.Session.Get(r.Context(), fmt.Sprintf("block_map_%d", x.ID)).(map[string]int)
		for name, value := range curMap {
			//ok will be false if value not in map
			if val, ok := curMap[name]; ok {
				//only pay attention to values greater than 0 and not in form post
