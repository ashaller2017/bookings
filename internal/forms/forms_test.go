...
func TestForm_Require(t *testing.T) {
	r := httptest.NewRequest("POST", "/whatver", nil)
	form := New(r.PostForm)
	form.Required("a", "b", "c")
	if form.Valid() {
...
func TestForm_Has(t *testing.T) {
	...
	has = form.Has("a")
	if !has {
		t.Error("shows form doesnt have field when it does")
	}
...
func TestForm_MinLen(t *testing.T) {
	...
	if isError == "" {
		t.Error("should have an error but didt get one")
	}
	...
	if !form.Valid() {
		t.Error("shows minlength of 1 is not met when is")
	}
...
func TestForm_Email(t *testing.T) {
	...
	form.IsEmail("email")
	if !form.Valid() {
		t.Error("got an invalid email when we shold not have")
	}
	postedValues = url.Values{}
